...
// Check if a movie is already pinned
function isMoviePinned(movie, pinnedMovies) {
...
function togglePin(movie) {
    ...
    const isPinned = isMoviePinned(movie, pinnedMovies);

    if (isPinned) {
        // Remove the movie if it's already pinned
        const updatedPinnedMovies = pinnedMovies.filter((m) => m.title !== movie.title);
        savePinnedMoviesToStorage(updatedPinnedMovies);
    } else {
        // Add the movie if it's not pinned
